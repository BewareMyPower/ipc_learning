...
enum MQ_METHOD { MQ_NONE, MQ_CREATE = 1, MQ_INFO, MQ_REMOVE };
...
int main(int argc, char* argv[]) {
  ...
  if (optind >= argc) UsageError(argv[0], "mq-name must be specified");
  ...
  switch (mq_method) {
    ...
    case MQ_REMOVE:
      if (mq_unlink(mq_name)) errorExit(errno, "mq_unlink");
