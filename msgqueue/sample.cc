...
mq_create_again:
  ...
  if (mqd == -1)
    errorExit(errno, "mqd_open \"%s\" O_WRONLY | O_NONBLOCK", mq_name);
  ...
  if (mqd == -1)
    errorExit(errno, "mqd_open \"%s\" O_RDONLY | O_NONBLOCK", mq_name);

  if (mq_getattr(mqd, &attr) == -1) errorExit(errno, "mq_getattr");

  char* buffer = new char[attr.mq_msgsize + 1];
  unsigned int priority;
  while ((num_read = mq_receive(mqd, buffer, attr.mq_msgsize, &priority)) >=
         0) {
    buffer[num_read] = '\0';
    printf("[%2u] %s\n", priority, buffer);
  }
